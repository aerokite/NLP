...
class Word2Vec:
    ...
    def skip_gram(self, train_x, train_y, w1, w2):
        # forward propagation
        h = np.dot(w1.T, train_x)
        u = np.dot(w2.T, h)
        yp = softmax(u)

        # error calculation
        ep = np.sum(np.subtract(yp, train_y), axis=0)

        # backward propagation
        delta_h = np.outer(train_x, np.dot(w2, ep))
        ...
        # loss calculation
        loss = -np.sum(u[ty.index(1)] for ty in train_y) + len(train_y) * np.log(np.sum(np.exp(u)))
        return nw1, nw2, loss
    ...
    def run(self, check_words=None):
        ...
        for e in range(self.epochs):
            ...
            for center, neighbors in pairs:
                W1, W2, loss = self.skip_gram(center, neighbors, W1, W2)
                e_loss += loss
